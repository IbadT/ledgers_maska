┌─────────────────┐ │ MaskaInput │ │ (от Matematika)│ └────────┬────────┘ │ ▼
┌─────────────────┐ ┌──────────────────┐ │ MaskaService │────▶│ validateInput()
│ │ │ └──────────────────┘ │ - sort txs │ │ - process each │
┌──────────────────┐ │ transaction │────▶│ ContractorsService│
└────────┬────────┘ │ - load CSV │ │ │ - merge custom │ │ └──────────────────┘ │
│ │ ┌────────▼─────────┐ │ │ TemplateService │ └─────────────▶│ - resolve │ │
placeholders │ └────────┬─────────┘ │ ┌────────────┼────────────┐ ▼ ▼ ▼
┌──────────┐ ┌──────────┐ ┌──────────┐ │Generators│ │ CSV │ │ JSON │ │ Service │
│ Loaders │ │ Templates│ │(ID gen) │ │ │ │ │ └──────────┘ └──────────┘
└──────────┘ │ ▼ ┌─────────────────┐ │ MaskaOutput │ │ (description) │
└─────────────────┘


```bash
npm run format:check    # Проверить форматирование
npm run lint:check      # Проверить линтинг
